...
public class Download extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  File file = new File(path);    // new File("D:\storage2\404.jpg") -> 리눅스에서는 /사용
	  
	  // 다운로드 할 파일을 읽어 들일 바이트 기반 입력 스트림 생성
	  BufferedInputStream in = new BufferedInputStream(new FileInputStream(file));
	  
	  // 다운로드용 응답을 만드려면 응답 헤더 "Content-Disposition"에 "attachment" 값을 저장해야 한다.
	  response.setHeader("Content-Disposition", "attachment");   // 다운로드할 때 파일 저장 대화상자가 나타난다. 어디에 저장할지 창이 나오는 방식
	  ...
	  // 다운로드할 때 대화상자가 나타나지 않는다. 지정하지 않으면 크롬의 기본 다운로드 디렉토리로 간다.
	  // 직접 지정 : split으로 /기준 배열로 만든 뒤 맨 마지막 것만 가져오거나, substring을 이용해서 마지막 /(슬래쉬) 다음부터 끝까지 가져오기
