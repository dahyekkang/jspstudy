...
<script>
  ...
  function fnMemberList(){
    $.ajax({
      ...
      success: function(resData){
        var obj = JSON.parse(resData);    // js객체로 바꿈
        // obj.memberList
  ...
  function fnInit(){
    ...
    // $('#btn_init').click(fnInitDetail); 와 같다.
  }
  ...
  function fnMemberAdd(){
    $('#btn_add').click(function(){
      ...
      $.ajax({
        ...
        data: $('#frm_member').serialize(), // 폼의 모든 입력 요소를 파라미터로 전송함(입력 요소는 name 속성이 필요함) 서브밋이 아니고 ajax요청이다.
        dataType: 'text',
    ...
    $('#btn_delete').click(function(){
      ...
      $.ajax({
        ...
        success: function(resData){       // resData === '{"deleteResult":1}'
          var obj = JSON.parse(resData);  //     obj === {"deleteResult":1}   -> 자바스크립트 객체가 된다.
  ...
  <div class="wrap">
    <form id="frm_member">
      ...
      <div>
        ...
        <button type="button" id="btn_init">입력초기화</button>   <!-- submit은 ajax에서 사용할 수 없다. -->
