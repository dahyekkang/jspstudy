...
public class MemberDao {
  ...
  private MemberDao() {
    ...
    try {
      ...
      factory = new SqlSessionFactoryBuilder().build(in);   // 읽어들인 입력스트림의 내용으로 팩토리를 만든다.
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public int memberModify(MemberDto dto) {
    SqlSession ss = factory.openSession(false);   // update는 autocommit 설정 해야 함
  ...
  public int memberDelete(int memberNo) {
    SqlSession ss = factory.openSession(false);   // delete는 autocommit 설정 해야 함!
