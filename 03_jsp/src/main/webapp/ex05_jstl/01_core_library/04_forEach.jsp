...
<body>
  ...
  <%
    List<BlogPost> list = Arrays.asList(
      new BlogPost(1, "첫 번째 블로그", 10, LocalDate.of(2023, 1, 1)),
      new BlogPost(2, "두 번째 블로그", 15, LocalDate.of(2023, 1, 2)),
      new BlogPost(3, "세 번째 블로그", 20, LocalDate.of(2023, 1, 3))
    );
    pageContext.setAttribute("list", list);
  %>
  <c:forEach var="blogPost" items="${list}">
    <div class="blog">
      <span class="blogPostNo">블로그번호: ${blogPost.blogPostNo}<br></span>
      <span class="title">제목: <a href="">${blogPost.title}</a><br></span>
      <span class="hit">조회수: ${blogPost.hit}<br></span>
      <span class="createdAt">작성일: ${blogPost.createdAt}<br><br></span>
    </div>
  </c:forEach>
