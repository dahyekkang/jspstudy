...
public class RememberMe extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // System.out.println(remember_me);   // value가 없을 때, checkbox 체크하면 디폴트값(on)이 넘어온다. 체크 안 하면 null값이 넘어온다.
	  
	  
	  // 아이디 기억하기 체크했다면 쿠키게 id를 저장한다.
	  Cookie cookie = null;
	  if(remember_me != null) {    // if(remember_me.equals("on")){ 과 동일함
	    ...
	    cookie = new Cookie("remember_me", "");    // 값은 의미 없다.
